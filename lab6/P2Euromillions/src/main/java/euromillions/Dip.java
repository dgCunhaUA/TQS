 ...
 */
public class Dip {
    static final int NUMBERS_MAXRANGE = 49;
    static final int STARTS_MAXTANGE = 9;
    static final double N_NUMBERS = 5;
    static final double N_STARTS = 2;
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < N_NUMBERS; i++) {
            int candidate = secureRandom.nextInt(NUMBERS_MAXRANGE) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
            }
        }
        for (int i = 0; i < N_STARTS; i++) {
            int candidate = secureRandom.nextInt(STARTS_MAXTANGE) + 1;
